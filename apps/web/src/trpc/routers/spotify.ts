...
export const spotifyRouter = createTRPCRouter({
  get: publicProcedure.query(async ({ ctx }) => {
    ...
    const rateLimitResponse = await ratelimit.limit(getKey(ip)).catch((error: unknown) => {
      console.log('Error while ratelimit,', error)
      return null
    })
    if (!rateLimitResponse?.success) throw new TRPCError({ code: 'TOO_MANY_REQUESTS' })
