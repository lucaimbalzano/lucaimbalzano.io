...
import { Comment, Reply } from '@simbashrd/emails'
...
export const commentsRouter = createTRPCRouter({
  ...
  post: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const post = {
        title,
        url: `https://lucaimbalzano.io/blog/${input.slug}`
      }
      ...
      await ctx.db.transaction(async (tx) => {
        ...
        if (!input.parentId && user.role === 'user') {
          ...
          await resend.emails.send({
            from: 'lucaimbalzano.io <lucaimbalzano@gmail.com>',
            to: process.env.AUTHOR_EMAIL ?? 'lucaimbalzano@gmail.com',
